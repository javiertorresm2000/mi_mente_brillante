...
<q-card class="bg-azul0 q-pa-lg column no-wrap amarillo" style="border-radius: 15px; gap:20px; background-color: #28363D;" :style="estiloReg">
    ...
    <div class="col-1 row items-center q-pb-none" style="position: relative;">
        <div class=" text-h5 fuente-subtitulos text-bold t-amarillo ">Registro</div>
    </div>
    ...
    <div class="column no-wrap" style="gap: 10px;" v-if="!usuario.usuario.reg_success">
        ...
        <div class="row" style="gap:10px">
            <q-input class="col" dense outlined v-model="nombre" label="Nombre" bg-color="white" :rules="[val => !!val || 'Dato requerido']"  :disable="usuario.usuario.loading1" />
            <q-input class="col" dense outlined v-model="apellido" label="Apellido" bg-color="white" :rules="[val => !!val || 'Dato requerido']" :disable="usuario.usuario.loading1" />
        </div>
        <div class="row" style="gap:10px;">
        ...
        <q-input dense outlined v-model="mail" label="Correo" bg-color="white" debounce="1500" :error="mailError" :rules="[val => !!val || 'Dato requerido', isValidEmail,]" :disable="usuario.usuario.loading1" >
            <template v-slot:error>
                    {{mensaje}}
                </template>
        </q-input>
        
      <q-input v-model="pass2" outlined :type="isPwd1 ? 'password' : 'text'" label="Contraseña" dense bg-color="white" :disable="usuario.usuario.loading1" >
        <template v-slot:append>
            <q-icon
                :name="isPwd1 ? 'fas fa-eye-slash' : 'fas fa-eye'"
      ...
      <q-input v-model="pass1" outlined :type="isPwd ? 'password' : 'text'" label="Repetir contraseña " dense bg-color="white" :error="error" :disable="usuario.usuario.loading1" >
        <template v-slot:append>
            <q-icon
      ...
      <q-btn class="full-width bg-amarillo text-white"  label="Registrarme" v-on:click="registro" :loading="usuario.usuario.loading1" :disable="usuario.usuario.loading1" ></q-btn>
    </div>
