...
<template>
   ...
   <div class="text-weight-bold text-h5 " :class="ui.device==='smartphone'?'q-pa-sm':'q-pa-lg'">Programas</div>
   <template v-if="cursos.total_adquiridos==0">
    ...
    <div class=" row  wrap justify-center q-pt-md" style="gap: 40px;" :class="ui.device==='smartphone'?'q-pa-sm':'q-pa-lg'">
      ...
      <template v-for="c in cursos.cursos" v-if="!cursos.loading">
        <template v-if="(c.adquirido)">
          <TarjetaCurso :categoria="c.categoria" :titulo="c.titulo" :thumb="c.thumb" :id="c.id" :modulo="c.modulo" :comprar="c.adquirido" :stepsTotal="c.stepsTotal" :stepsCompleted="c.stepsCompleted" :url_compra="c.url_compra" ></TarjetaCurso>
        </template>
      </template>
      <template v-else >
        <div class="q-pt-xl">
    ...
    <template  v-if="!cursos.loading">
      ...
      <div  class="text-weight-bold text-h5" :class="ui.device==='smartphone'?'q-pa-sm':'q-pa-lg'">Programas disponibles </div>
